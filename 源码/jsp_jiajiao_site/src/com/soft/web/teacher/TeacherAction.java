...
public class TeacherAction extends ActionSupport implements ServletRequestAware{
	...
	//2016-7-21添加start教员个人信息
	 public String TeacherInfo(){
			...
			request.getSession().setAttribute("img", listinfo.getImages());
			request.getSession().setAttribute("tname", listinfo.getT_name());
			request.getSession().setAttribute("money", listinfo.getMoney());
			request.getSession().setAttribute("subject", listinfo.getSubject());
			request.getSession().setAttribute("duce", listinfo.getIntroduce());
			request.getSession().setAttribute("ttell", listinfo.getTell());
